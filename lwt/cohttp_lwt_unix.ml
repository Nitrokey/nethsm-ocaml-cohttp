...
module Server = struct
  ...
  let create ?stop ?timeout ~port ?(mode=`TCP (`Port port)) spec =
    Lwt_unix_conduit.Server.serve ?stop ?timeout mode
...
module type S = sig
  ...
  val create : ?stop:(unit Lwt.t) -> ?timeout:int -> port:int -> ?mode:Conduit.Server.t -> t -> unit Lwt.t
end
